# Aan de slag

dGENIX is in 5 minuten actief. Je werkt via het dashboard op [app.dgenix.com](https://app.dgenix.com).

---

## Wat heb je nodig?

- Een e-mailadres voor je account
- Een toegangscode (in de early access fase)

Geen server, geen technische kennis, geen installaties.

---

## De stappen

### 1. Toegangscode aanvragen

dGENIX is momenteel in early access. Vraag een gratis toegangscode aan op [dgenix.com/invite](https://dgenix.com/invite). Je ontvangt hem binnen 24 uur.

→ [Hoe vraag ik een toegangscode aan?](toegangscode.md)

---

### 2. Account aanmaken

Ga naar [app.dgenix.com/signup](https://app.dgenix.com/signup), vul je e-mail, wachtwoord en toegangscode in en kies een abonnement.

→ [Account aanmaken](account-aanmaken.md)

---

### 3. Assistent instellen & activeren

Geef je AI-assistent een naam en persoonlijkheid via het dashboard. Bepaal hoe hij communiceert en jou het beste helpt.

→ [Assistent instellen](telegram-koppelen.md)

---

### 4. Integraties koppelen

Koppel Gmail, Google Calendar, Sheets en Drive — je assistent is meteen actief met jouw tools.

→ [Integraties koppelen](../integraties/gmail.md)

---

### 5. Skills activeren

Ga naar **Dashboard → Skills** en activeer de skills die je wilt gebruiken.

→ [Skills marketplace](../skills/README.md)

---

## Klaar — direct aan de slag

Je kunt direct beginnen via het dashboard. Stuur je eerste opdracht:

> Wil je je assistent ook via Telegram bereiken? Activeer de gratis **Telegram add-on** via Dashboard → Skills.

> *"Zoek 10 leads in de Amsterdamse horeca"*

> *"Geef me een overzicht van mijn ongelezen e-mails"*

> *"Maak een dagplanning voor morgen: vergadering 9u, presentatie 14u"*

Je assistent voert de taak uit en stuurt je het resultaat terug.
